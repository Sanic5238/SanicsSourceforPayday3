...
{
	const auto Id = AssetData.GetTagValueRef<FGuid>(GET_MEMBER_NAME_CHECKED(UAkAudioType, ID));
	if (!Id.IsValid())
	{
		return false;
	}

	if (FAssetData* CurrentObjectIt = Instance->AudioTypeMap.Find(Id))
	{
		const FString ObjectPath = AkUEGetObjectPathString(CurrentObjectIt);

		if (ObjectPath != AkUEGetObjectPathString(AssetData))
		{
			return false;
		}
	}
	...
	if (AssetClass->IsChildOf<UAkAcousticTexture>())
	{
		Instance->AcousticTextureMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkAudioEvent>())
	{
		Instance->EventMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkAuxBus>())
	{
		Instance->AuxBusMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkGroupValue>())
	{
		Instance->GroupValueMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkTrigger>())
	{
		Instance->TriggerMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkRtpc>())
	{
		Instance->RtpcMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkAudioBank>())
	{
		Instance->BankMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkFolder>())
	{
		Instance->FolderMap.Remove(Id);
	}
	else if (AssetClass->IsChildOf<UAkInitBank>())
	{
