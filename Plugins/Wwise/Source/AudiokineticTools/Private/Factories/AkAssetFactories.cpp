...
UObject* UAkExternalSourceFactory::FactoryCreateFile(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	{
		//For some reason, in commandlet mode in UE_5.1, GetAsset() won't fully load the package if it is in a semi-loaded state, so we have to manually load it with GetPackage()
		...
		if (!mediaInstance)
		{
			UE_LOG(LogAkAssetFactory, Display, TEXT("UAkExternalSourceFactory: Failed to load asset %s."), *assetData.GetFullName());
		}
	}
	else
	{
		UE_LOG(LogAkAssetFactory, Display, TEXT("UAkExternalSourceFactory: Couldn't find asset %s. Trying to create a new one."), *assetPath);
		...
		{
			{
				...
				{
					if (chunk.Data.IsValid() && chunk.Data->IsLocked())
					{
						chunk.Data->Unlock();
					}
				}
			...
			dataChunk->Data = TSharedPtr<FByteBulkData, ESPMode::ThreadSafe>(new FByteBulkData());
			dataChunk->Data->SetBulkDataFlags(BULKDATA_Force_NOT_InlinePayload);

			dataChunk->Data->Lock(EBulkDataLockFlags::LOCK_READ_WRITE);

			if(void* rawData = dataChunk->Data->Realloc(fileSize))
			{
				fileReader->Read(reinterpret_cast<uint8*>(rawData), fileSize);
			}
			else
			{
				UE_LOG(LogAkAssetFactory, Error, TEXT("Failed to allocate memory for dataChunk for external source file '%s'."), *Filename);
			}
