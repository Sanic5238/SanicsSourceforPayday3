...
void AkSoundDataBuilder::CookMediaAsset(MediaAssetToCook& MediaToCook, const FString& Platform)
{
	...
	bool bPrefetchSizeDifferent = false;
	if (PlatformData->DataChunks.Num() > 0 && !PlatformData->DataChunks[0].Data.IsValid() )
	{
		UE_LOG(LogAkAudio, Error, TEXT("AkSoundDataBuilder::CookMediaAsset: Bulk data is invalid, asset will be updated. Name: %s, ID: %u."), *MediaToCook.MediaName, MediaToCook.Id);
		bPrefetchSizeDifferent= true;
	}
	else
	{
		bPrefetchSizeDifferent = PlatformData->IsStreamed && PlatformData->DataChunks.Num() > 0 && PlatformData->DataChunks[0].IsPrefetch && PlatformData->DataChunks[0].Data->GetBulkDataSize() != ParsedPlatformData->PrefetchSize;
	}
	...
	bool BulkDataTypeIsOutOfDate = false;
	{
		...
		{
			if (!DataChunk.Data.IsValid() || ((DataChunk.Data->GetBulkDataFlags() & BULKDATA_ForceInlinePayload) != 0))
			{
...
#if UE_5_1_OR_LATER
				FString AssetPackagePath = FPaths::GetPath(MediaToCook->AssetPath.GetAssetPath().ToString());
#else
				FString AssetPackagePath = FPaths::GetPath(MediaToCook->AssetPath.GetAssetPathName().ToString());
#endif
				auto AssetData = AkUEGetAssetByObjectPath(MediaToCook->AssetPath);
