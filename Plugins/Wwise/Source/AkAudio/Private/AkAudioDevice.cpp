...
namespace FAkAudioDevice_Helpers
{
	...
	FAkAudioDeviceDelegates::FOnAkMonitoringCallback AkMonitoringDelegate;
	...
	void MonitoringCallback(const AkResourceMonitorDataSummary* DataSummary)
	{
		if (AkMonitoringDelegate.IsBound())
		{
			AkMonitoringDelegate.Broadcast(DataSummary);
		}
	}
}
...
FDelegateHandle FAkAudioDevice::RegisterResourceMonitorCallback(FAkAudioDeviceDelegates::FOnAkMonitoringCallback::FDelegate Callback)
{
	//First delegate to register will also register the callback with the sound engine
	if (!FAkAudioDevice_Helpers::AkMonitoringDelegate.IsBound())
	{
		AK::SoundEngine::RegisterResourceMonitorCallback(FAkAudioDevice_Helpers::MonitoringCallback);
	}

	FDelegateHandle Handle = FAkAudioDevice_Helpers::AkMonitoringDelegate.Add(Callback);
	return Handle;
}

void FAkAudioDevice::UnregisterResourceMonitorCallback(FDelegateHandle DelegateHandle)
{
	FAkAudioDevice_Helpers::AkMonitoringDelegate.Remove(DelegateHandle);

	if (!FAkAudioDevice_Helpers::AkMonitoringDelegate.IsBound())
	{
		//If nothing is bound we can safely unregister the callback
		AK::SoundEngine::UnregisterResourceMonitorCallback(FAkAudioDevice_Helpers::MonitoringCallback);
	}
}
...
)
{
	...
	if (m_bSoundEngineInitialized && CallbackManager)
	{
		...
		if (pPackage)
		{
			...
			{
				UE_LOG(LogAkAudio, VeryVerbose, TEXT("FAkAudioDevice::PostEvent : Posted Event %s. Playing ID is %d."), *in_EventName, playingID);
				FScopeLock Lock(&EventToPlayingIDMapCriticalSection);
...
)
{
	...
	if (m_bSoundEngineInitialized && in_pGameObject && CallbackManager)
	{
		if (in_pGameObject->VerifyEventName(in_EventName) && in_pGameObject->AllowAudioPlayback())
		{
			...
			PlayingID = PostEvent(in_EventName, gameObjID, in_ExternalSources, CreateCallbackPackage, in_AudioContext);
			if (PlayingID != AK_INVALID_PLAYING_ID)
			{
				UE_LOG(LogAkAudio, VeryVerbose, TEXT("AkAudioDevice::PostEvent: Event %s posted with PlayingID %d. Setting bEventPosted flag on AkGameObject %s."), *in_EventName, PlayingID, *in_pGameObject->GetName());
				in_pGameObject->EventPosted();
			}
		}
	}
...
)
{
	...
	TWeakObjectPtr<UAkComponent> WeakAkComponent = TWeakObjectPtr<UAkComponent>(GetAkComponent(Actor->GetRootComponent(), FName(), NULL, EAttachLocation::KeepRelativeOffset));
	TWeakObjectPtr<AActor> WeakActor = TWeakObjectPtr<AActor>(Actor);
	TWeakObjectPtr<UAkAudioEvent> WeakEvent = TWeakObjectPtr<UAkAudioEvent>(AudioEvent);
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, WeakEvent, WeakActor, WeakAkComponent, PostEventCallback, CallbackFlags, bStopWhenOwnerDestroyed, ExternalSources, PlayingIDPromiseCopy(MoveTemp(PlayingIDPromise)), AudioContext]() mutable
		{
			AkPlayingID PlayingID = AK_INVALID_PLAYING_ID;

			if (m_bSoundEngineInitialized && WeakEvent.IsValid() && WeakActor.IsValid() && WeakAkComponent.IsValid())
			{

				WeakAkComponent->StopWhenOwnerDestroyed = bStopWhenOwnerDestroyed;

				PlayingID = PostEvent(WeakEvent->GetName(), WeakAkComponent.Get(), PostEventCallback, CallbackFlags, ExternalSources->ExternalSourceArray, AudioContext);

				if (PlayingID != AK_INVALID_PLAYING_ID)
				{
					WeakEvent->PinInGarbageCollector(PlayingID);
					for (auto &ExtSrc : ExternalSources->MemoryLoadedMedia)
					{
						if (ExtSrc.IsValid())
						{
							ExtSrc->AddPlayingIDAndPinInGarbageCollector(FAkAudioDevice::GetIDFromString(WeakEvent->GetName()), PlayingID);
						}
						else
						{
							UE_LOG(LogAkAudio, Warning, TEXT("PostEventAsync: Invalid external source passed to function."));
						}
					}
				}
			}
			else if (m_bSoundEngineInitialized)
			{
				UE_LOG(LogAkAudio, Warning, TEXT("Event was not posted due to invalid UObjects in async callback."));
			}
...
{
	TWeakObjectPtr<UAkGameObject> WeakGameObject = TWeakObjectPtr<UAkGameObject>(GameObject);
	TWeakObjectPtr<UAkAudioEvent> WeakEvent = TWeakObjectPtr<UAkAudioEvent>(AudioEvent);
	auto PlayingIDFuture = Async(EAsyncExecution::TaskGraphMainThread, [WeakEvent] {
		while (WeakEvent.IsValid() && !WeakEvent->IsLocalizationReady())
		{
			FPlatformProcess::Sleep(1.f / 60.f);
		}
	}).Then([this, WeakEvent, WeakGameObject, PostEventCallback, CallbackFlags, ExternalSources, AudioContext](auto PreviousFuture) {
		if (!WeakEvent.IsValid() || !WeakGameObject.IsValid())
		{
			return AK_INVALID_PLAYING_ID;
		}
		AkPlayingID PlayingID = PostEvent(WeakEvent->GetName(), WeakGameObject.Get(), ExternalSources->ExternalSourceArray,
...
)
{
	TPromise<AkPlayingID> playingIDPromise;
	auto playingIDFuture = playingIDPromise.GetFuture();
	TWeakObjectPtr<UWorld> WeakWorld = TWeakObjectPtr<UWorld>(in_World);
	TWeakObjectPtr<UAkAudioEvent> WeakEvent = TWeakObjectPtr<UAkAudioEvent>(in_pEvent);
	auto pollMediaReadyTask = FFunctionGraphTask::CreateAndDispatchWhenReady([WeakEvent]()
		{
			while (WeakEvent.IsValid() && !WeakEvent->IsLocalizationReady())
			{
				FPlatformProcess::Sleep(1.f / 60.f);
	...
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, WeakEvent, in_Location, in_Orientation, WeakWorld, playingIDPromiseCopy(MoveTemp(playingIDPromise)), in_AudioContext]() mutable {
		AkPlayingID playingID = AK_INVALID_PLAYING_ID;

		if (WeakEvent.IsValid() && WeakWorld.IsValid())
		{
			playingID = PostEventAtLocation(WeakEvent->GetName(), in_Location, in_Orientation, WeakWorld.Get(), in_AudioContext);
			if (playingID != AK_INVALID_PLAYING_ID)
			{
				WeakEvent->PinInGarbageCollector(playingID);
			}
...
)
{
	...
	TWeakObjectPtr<UAkComponent> WeakAkComponent = TWeakObjectPtr<UAkComponent>(GetAkComponent(Actor->GetRootComponent(), FName(), NULL, EAttachLocation::KeepRelativeOffset));
	TWeakObjectPtr<AActor> WeakActor = TWeakObjectPtr<AActor>(Actor);
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, WeakEvent, WeakActor, WeakAkComponent, bStopWhenOwnerDestroyed, LatentAction, ExternalSources, PlayingIDPromiseCopy(MoveTemp(PlayingIDPromise)), in_AudioContext]() mutable {
		AkPlayingID PlayingID = AK_INVALID_PLAYING_ID;
		...
		if (m_bSoundEngineInitialized && WeakEvent.IsValid() && WeakActor.IsValid() &&WeakAkComponent.IsValid())
		{
			WeakAkComponent->StopWhenOwnerDestroyed = bStopWhenOwnerDestroyed;
			PlayingID = PostEvent(
				WeakEvent->GetName(), WeakAkComponent.Get(), ExternalSources->ExternalSourceArray,
			...
			if (PlayingID != AK_INVALID_PLAYING_ID)
			{
				...
				for (auto &ExtSrc : ExternalSources->MemoryLoadedMedia)
				{
					...
					{
						UE_LOG(LogAkAudio, Warning, TEXT("PostEventLatentActionAsync: Invalid external source passed to function."));
					}
				}
			}
		}
...
{
	TWeakObjectPtr<UAkAudioEvent> WeakEvent = TWeakObjectPtr<UAkAudioEvent>(AudioEvent);
	TWeakObjectPtr<UAkComponent> WeakAkComponent= TWeakObjectPtr<UAkComponent>(AkComponent);

	auto PlayingIDFuture = Async(EAsyncExecution::TaskGraph, [WeakEvent]
		{
			while (WeakEvent.IsValid() && !WeakEvent->IsLocalizationReady())
			{
				FPlatformProcess::Sleep(1.f / 60.f);
			}
		}).Then([this, WeakEvent, WeakAkComponent, LatentAction, ExternalSources, in_AudioContext](auto PreviousFuture)
		{
			if (m_bSoundEngineInitialized && WeakEvent.IsValid() && WeakAkComponent.IsValid())
			{
				AkPlayingID PlayingID = PostEvent(WeakEvent->GetName(), WeakAkComponent.Get(), ExternalSources->ExternalSourceArray,
					[LatentAction, this](AkGameObjectID GameObjectID) 
					{
						return CallbackManager->CreateCallbackPackage(LatentAction, GameObjectID);
					}, in_AudioContext);
				if (PlayingID != AK_INVALID_PLAYING_ID)
				{
					...
					for (auto& ExtSrc : ExternalSources->MemoryLoadedMedia)
					{
						if (ExtSrc.IsValid())
						{
							UE_LOG(LogAkAudio, VeryVerbose, TEXT("PostEventLatentActionAsync: Pinning Media for event %s with playing ID %d."), *WeakEvent->GetName(), PlayingID);
							ExtSrc->AddPlayingIDAndPinInGarbageCollector(FAkAudioDevice::GetIDFromString(WeakEvent->GetName()), PlayingID);
						}
						else
						{
...
UAkComponent* FAkAudioDevice::GetAkComponent( class USceneComponent* AttachToComponent, FName AttachPointName, const FVector * Location, EAttachLocation::Type LocationType, bool& ComponentCreated )
{
	...
	if( GEngine && AK::SoundEngine::IsInitialized())
	{
		...
		if (AttachToComponent->IsValidLowLevel() )
		{
			Actor = AttachToComponent->GetOwner();
		}
		if ( Actor ) 
		{
			if( !IsValid(Actor) )
...
void FAkAudioDevice::CleanPinnedObjects(uint32 PlayingID)
{
	...
	UE_LOG(LogAkAudio, VeryVerbose, TEXT("FAkAudioDevice::CleanPinnedObjects Cleaning pinned objects for PlayingID : %d"), PlayingID);
	{
		FScopeLock Lock(&PlayingIDToPinnedAudioEventMapCriticalSection);
		...
		if (EventSetPtr)
		{
			...
			for (auto AkEvent : EventSet)
			{
				...
				{
					UE_LOG(LogAkAudio, Warning, TEXT("FAkAudioDevice::CleanPinnedObjects: AkEvent was destroyed before being unpinned from the GC. PlayingID : %d"), PlayingID);
				}
			}
			EventSet.Empty();
...

	{
		...
		if (MediaSetPtr)
		{
			...
			for (auto Media : MediaSet)
			{
				...
				{
					UE_LOG(LogAkAudio, Warning, TEXT("FAkAudioDevice::CleanPinnedObjects: Media was destroyed before being unpinned from the GC. PlayingID : %d"), PlayingID);
				}
			}
			MediaSet.Empty();
