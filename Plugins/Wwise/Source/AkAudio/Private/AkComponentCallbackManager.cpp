...
void FAkComponentCallbackManager::AkComponentCallback(AkCallbackType in_eType, AkCallbackInfo* in_pCallbackInfo)
{
	...
	if (Instance && pPackage)
	{
		...
		{
			UE_LOG(LogAkAudio, VeryVerbose, TEXT("FAkComponentCallbackManager::AkComponentCallback EndOfEvent received for EventID %d - PlayingID %d"), ((AkEventCallbackInfo*)in_pCallbackInfo)->eventID, ((AkEventCallbackInfo*)in_pCallbackInfo)->playingID);
			deletePackage = true;
