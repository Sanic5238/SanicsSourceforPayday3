		...
		if (ExternalSources.Num() > 0)
		{
			...
			if (playingID != AK_INVALID_PLAYING_ID)
			{
				for (auto& ExtSrc : ExternalSources)
				{
					...
					{
						ExtSrc.ExternalSourceAsset->AddPlayingIDAndPinInGarbageCollector(FAkAudioDevice::GetIDFromString(in_EventName), playingID);
					}
				}
			}
		}
...
)
{
	...
	if (!NewAction)
	{
		NewAction = new FPostAssociatedEventAction(LatentInfo, &PlayingID, ExternalSources, AkEvent, &bEventPosted);
		if (ExternalSources.Num() > 0)
		{
...
bool UAkGameObject::AllowAudioPlayback() const
{
	...
	return (CurrentWorld && IsValid(CurrentWorld) && !CurrentWorld->bIsTearingDown && CurrentWorld->AllowAudioPlayback());
}
...
{
	UE_LOG(LogAkAudio, VeryVerbose, TEXT("UAkGameObject::Stop: Stopping all Events posted on GameObjectID %llu."), GetAkGameObjectID());
