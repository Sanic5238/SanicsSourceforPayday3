		...
		if (ExternalSources.Num() > 0)
		{
			...
			if (PlayingID != AK_INVALID_PLAYING_ID)
			{
				for (auto& ExtSrc : ExternalSources)
				{
					if (ExtSrc.ExternalSourceAsset)
					{
						ExtSrc.ExternalSourceAsset->AddPlayingIDAndPinInGarbageCollector(FAkAudioDevice::GetIDFromString(GET_AK_EVENT_NAME(AkEvent, in_EventName)), PlayingID);
					}
				}
			}
		}
...
void UAkComponent::OnUnregister()
{
	...
	Super::OnUnregister();
	UE_LOG(LogAkAudio, VeryVerbose, TEXT("UAkComponent::OnUnregister: AkComponent %s is being unregistered."), *GetName());
