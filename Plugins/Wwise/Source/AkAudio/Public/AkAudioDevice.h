...
struct AKAUDIO_API FAkAudioDeviceDelegates
{
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnAkMonitoringCallback,const AkResourceMonitorDataSummary*);
};
...
public:
	...
	*/
	...
	/** 
	 * Registers a callback function that gets Wwise resource usage data 
	 *
	 * This function fail to register in Release
	 *
	 * @param Callback	The callback that will be called.
	*/
	FDelegateHandle RegisterResourceMonitorCallback(FAkAudioDeviceDelegates::FOnAkMonitoringCallback::FDelegate Callback);

	/**
	 * Unregisters a callback that was registered by RegisterResourceMonitorCallback
	 *
	 * @param Handle		The handle of the registered callback
	 */
	void UnregisterResourceMonitorCallback(FDelegateHandle DelegateHandle);
